...
Route53 is a DNS is a Domain Name Service _think_ Godaddy or NameCheap but with more synergies with AWS services.
...
For example if you has a record for `www.exampro.co` with 3 different IP address values, users would be directly randomly to 1 of them when visiting the domain.

`www.exampro.co` ==> 34.229.79.211
                     18.212.245.88
                      3.208.76.48
...
Allow you to direct traffic based on the geographic location of where the request orginated from.

For example this would let you route all traffic comming from North America to servers located in North America regions, where queries from other regions could be directed to servers hosted in that region.(potentially with pricing and language specific to that region)
...
* A health check can intial a failover if status is returned unhealthy.
...
* Health checks can be created to monitor and automatically over endpoints.